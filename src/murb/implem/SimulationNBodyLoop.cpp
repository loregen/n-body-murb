...
void SimulationNBodyLoop::computeBodiesAcceleration()
{
    ...
    for (unsigned long iBody = 0; iBody < N; iBody++) {
        // flops = n * 20
        for (unsigned long jBody = 0; jBody < N; jBody+=10) {
            float rijx = d[jBody].qx - d[iBody].qx; // 1 flop
            float rijy = d[jBody].qy - d[iBody].qy; // 1 flop
            float rijz = d[jBody].qz - d[iBody].qz; // 1 flop

            // compute the || rij ||² distance between body i and body j
            float rijSquared = rijx * rijx + rijy * rijy + rijz * rijz; // 5 flops
            // compute the acceleration value between body i and body j: || ai || = G.mj / (|| rij ||² + e²)^{3/2}
            float ai = this->G * d[jBody].m / ((rijSquared + softSquared) * std::sqrt(rijSquared + softSquared)); // 5 flops
            ...
            rijx = d[jBody].qx - d[iBody].qx; // 1 flop
            rijy = d[jBody].qy - d[iBody].qy; // 1 flop
            rijz = d[jBody].qz - d[iBody].qz; // 1 flop

            // compute the || rij ||² distance between body i and body j
            rijSquared = rijx * rijx + rijy * rijy + rijz * rijz; // 5 flops
            // compute the acceleration value between body i and body j: || ai || = G.mj / (|| rij ||² + e²)^{3/2}
            ai = this->G * d[jBody].m / ((rijSquared + softSquared) * std::sqrt(rijSquared + softSquared)); // 5 flops
